...
const POST_TYPES = [
  ...
  { value: 'CoreDrill', label: 'Core Drill' },
];

const INFILL_STYLES = [
  { value: 'Full',   label: 'Full Slat' },
  { value: '3-Rail', label: '3-Rail' },
];

// End types match PHP plugin: 'post' | 'half_post' | 'wall'
const END_TYPES = [
...
export default function AireCalculator() {
  ...
  const [postType,     setPostType]     = useState(AIRE_DEFAULTS.postType);
  const [infillStyle,  setInfillStyle]  = useState(AIRE_DEFAULTS.infillStyle);

  // Runs (up to 6)
  const [runs, setRuns] = useState(() => {
    const r = defaultAireRun(0);
    return [r];
  });
  ...
  useEffect(() => {
    ...
    if (s) {
      ...
      if (s.infillStyle)  setInfillStyle(s.infillStyle);
  ...
  useEffect(() => {
    if (!hydrated) return;
    persistState({ colour, handrailType, postType, infillStyle, runs, activeRun, projectId, projectName, calculationId });
  }, [colour, handrailType, postType, infillStyle, runs, activeRun, projectId, projectName, calculationId, hydrated]);
  ...
  // ── BOM calculation ────────────────────────────────────────────────────────
  const opts = { colour, handrailType, postType, style: infillStyle, sharedCorners: 0 };
  ...
  const hasPricing = enrichedBOM.length > 0; // always show price columns — TBC shown for unknowns
    ...
    <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', padding: '24px 20px', minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ background: '#1e3a5f', borderRadius: 10, padding: '16px 18px', color: 'white' }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700 }}>Horizontal Slat Balustrade</div>
          <div style={{ fontSize: 12, opacity: 0.6, marginTop: 4 }}>
            Max 1800mm post centres · 65×16.5mm slats · 9mm gap
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 10, padding: 16, border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <select style={selectStyle} value={postType} onChange={(e) => setPostType(e.target.value)}>
              {POST_TYPES.map((p) => <option key={p.value} value={p.value}>{p.label}</option>)}
            </select>
          </div>

          <div>
            <label style={labelStyle}>Infill Style</label>
            <select style={selectStyle} value={infillStyle} onChange={(e) => setInfillStyle(e.target.value)}>
              {INFILL_STYLES.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
            </select>
          </div>
        </div>
          ...
          {runs[activeRun] && (
            <div style={{ padding: 16 }}>
              ...
              {runs[activeRun].length > 0 && (() => {
                const spans = Math.max(1, Math.ceil(runs[activeRun].length / runs[activeRun].maxPostSpan));
                const posts = spans + 1;
                const slatZone = runs[activeRun].height - runs[activeRun].bottomGap;
                const slatsPerPanel = slatZone > 0 ? Math.floor(slatZone / 74) : 0;
                return (
                  <div style={{ background: '#f0f9ff', borderRadius: 6, padding: '8px 12px', fontSize: 12, color: '#0369a1' }}>
                    {spans} panel{spans > 1 ? 's' : ''} · {posts} posts · {slatsPerPanel} slat rows/panel
                  </div>
                );
              })()}
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>
              AIRE+ Horizontal Slat · {colourLabel} · {handrailType} handrail · {postType === 'BasePlate' ? 'Base Plate' : 'Core Drill'} posts
            </div>
          </div>
          {hasPricing && (
        ...
        <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e7eb', padding: '14px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {runs.map((r, i) => {
              const spans = r.length > 0 ? Math.max(1, Math.ceil(r.length / r.maxPostSpan)) : 0;
              const posts = spans + 1;
              const slatsPerPanel = Math.floor((r.height - r.bottomGap) / 74);
              return (
                <div
                  key={i}
                  onClick={() => setActiveRun(i)}
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: '#9ca3af' }}>{posts} posts · {slatsPerPanel} slat rows</div>
                </div>
              );
            })}
          </div>
      ...
      <SaveProjectModal
        ...
        calculatorType="aire"
        calculatorState={{ colour, handrailType, postType, infillStyle, runs, activeRun }}
        bomSnapshot={getCurrentBom()}
        currentProjectId={projectId}
        currentProjectName={projectName}
        currentCalculationId={calculationId}
        label="AIRE+ Horizontal Slat"
      />
    </div>
  );
}
