...
import { shapeDefaults, buildIntersectionMap, getDefaultRunConfig } from "@/lib/solverEngine";
...
import { getDefaultBalustradeFinishes } from "@/lib/balustradeSkuMaps";
import TopDownPreview from "@/components/balustrade/TopDownPreview";
import RunSetupPanel from "@/components/balustrade/RunSetupPanel";
...
export default function Calculator({ demoMode = false }) {
  ...
  const [finishes, setFinishes] = useState(getDefaultBalustradeFinishes());
  ...
  useEffect(() => {
    setRuns(prev => {
      ...
      const next = prev.map((cfg, i) => {
        ...
        const allowDualCornerFlip = (shape === "U-shape" && i === 0);
    ...
    <div className="min-h-screen md:h-screen bg-gray-50 flex flex-col overflow-y-auto md:overflow-hidden">
      ...
      <header className="bg-white border-b border-gray-200 px-3 md:px-5 py-3 flex items-center gap-3 flex-shrink-0">
        ...
        <span className="text-sm font-bold text-gray-800 tracking-wide flex-1">
          Expert Fence Balustrade Calculator
